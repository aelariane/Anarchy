...
public class PhotonPlayerProperty
{
    ...
    private static void InitAllProprties()
    {
        var tmp = new System.Collections.Generic.List<string>();
        FieldInfo[] infos = typeof(PhotonPlayerProperty).GetFields(BindingFlags.Static | BindingFlags.Public);
        foreach(FieldInfo info in infos)
        {
            if(info.IsStatic && info.DeclaringType == typeof(string))
            {
                tmp.Add(info.GetValue(info).ToString());
            }
        }
